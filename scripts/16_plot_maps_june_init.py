"""
16_plot_maps_june_init.py

Purpose
-------
Plot NMME precipitation-anomaly maps for the June 2026 initialization,
for the periods actually available from that cycle: Jul, Aug, Sep, and
JAS (Jul-Aug-Sep). June, JJA, and JJAS are not produced -- see
15_compute_anomalies_june_init.py for why.

Inputs:
    outputs/netcdf/nmme_anomalies_june_init/NMME_prate_*_total_anomaly_*.nc

Outputs:
    outputs/maps/dynamic_diagnostics_june_init/prate_<region>_<period>.png

Uses the same diverging ochre (dry) <-> teal (wet) color convention as
the rest of the project's dashboard, rather than a generic colormap.

Run from project root:
    python scripts\\16_plot_maps_june_init.py
"""
...
NC_DIR = PROJECT_ROOT / "outputs" / "netcdf" / "nmme_anomalies_june_init"
OUT_DIR = PROJECT_ROOT / "outputs" / "maps" / "dynamic_diagnostics_june_init"
...
JOBS = [
    ("Jul",  "NMME_prate_Jul_2026_total_anomaly_mm_month.nc",  "prate_Jul_2026_total_mm_month",  "mm/month", "July 2026"),
    ...
    ("JAS",  "NMME_prate_JAS_2026_total_anomaly_mm_season.nc", "prate_JAS_2026_total_mm_season", "mm/season","July-Sep 2026 (JAS)"),
]
...
def main():
    print("==================================================")
    print("Plot NMME precipitation-anomaly maps -- June 2026 init")
    print("==================================================")
    ...
    for period, fname, varname, unit, plabel in JOBS:
        ...
        for region, meta in REGIONS.items():
            ...
            fig.text(0.5, 0.015, "CPC NMME ENSMEAN · initialized 2026-06-08 00Z · below normal (ochre) / above normal (teal)",
