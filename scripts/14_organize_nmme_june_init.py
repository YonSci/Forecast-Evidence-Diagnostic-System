"""
14_organize_nmme_june_init.py

Purpose
-------
Organize the downloaded June 2026 NMME ENSMEAN realtime-anomaly files
(from 13_download_nmme_june_init.py) the same way 03_inspect_and_organize
_fields.py organizes the May 2026 cycle: squeeze each file down to its
main data variable and save it into data/nmme/organized/.

Because output filenames are derived from the source filename (which
embeds "202606"), this does not collide with the existing May-cycle
("202605") organized files.

Run from project root:
    python scripts\\14_organize_nmme_june_init.py
"""
...
NMME_REALTIME_DIR = NMME_DIR / "realtime_anom" / "ENSMEAN" / "2026060800"
...
def main():
    print("==================================================")
    print("Organize June 2026 NMME realtime anomaly fields")
    print("==================================================")
    print(f"Source: {NMME_REALTIME_DIR}")
    print(f"Output: {NMME_ORG_DIR}")

    if not NMME_REALTIME_DIR.exists():
        raise FileNotFoundError(f"Not found: {NMME_REALTIME_DIR}. Run 13_download_nmme_june_init.py first.")
    ...
    for nc_file in nc_files:
        ...
        da.attrs["source_note"] = "CPC NMME realtime anomaly ENSMEAN file (June 2026 init)."
