"""
15_compute_anomalies_june_init.py

Purpose
-------
Compute NMME precipitation and surface-temperature anomalies for the
June 2026 initialization, following the same method as
04_compute_anomalies.py (mm/day conversion, monthly totals, seasonal
mean/total, cosine-latitude-weighted area means).

Target months
-------------
The June 2026 init's first available forecast target is JULY 2026 (the
init month itself, June, is skipped by the NMME real-time anomaly
product -- verified against target-month coordinates in
13_download_nmme_june_init.py). This script therefore only produces:
    Jul 2026, Aug 2026, Sep 2026, and JAS 2026 (Jul+Aug+Sep).
It does NOT produce June, JJA, or JJAS -- those require a June value
that does not exist in this initialization cycle.

Outputs
-------
Written to a namespace separate from the May-init outputs so nothing
in the existing (validated) May-init pipeline is overwritten:
    outputs/netcdf/nmme_anomalies_june_init/
    outputs/tables/nmme_area_mean_anomalies_june_init.csv

Run from project root:
    python scripts\\15_compute_anomalies_june_init.py
"""
...
NMME_PRATE_FILE = NMME_ORG_DIR / "nmme_NMME_prate_202606_ENSMEAN_anom.nc"
NMME_TMPSFC_FILE = NMME_ORG_DIR / "nmme_NMME_tmpsfc_202606_ENSMEAN_anom.nc"

NMME_ANOM_OUT_DIR = NETCDF_OUT_DIR / "nmme_anomalies_june_init"
NMME_ANOM_OUT_DIR.mkdir(parents=True, exist_ok=True)
TABLE_DIR.mkdir(parents=True, exist_ok=True)

# index 0 of the organized June-init file == July 2026 (see module docstring)
TARGET_MONTHS = {
    "Jul_2026": {"index": 0, "month": 7, "days": 31},
    "Aug_2026": {"index": 1, "month": 8, "days": 31},
    "Sep_2026": {"index": 2, "month": 9, "days": 30},
}
...
def select_target_months(da):
    ...
    for label, info in TARGET_MONTHS.items():
        selected[label] = da.isel(target=info["index"]).squeeze(drop=True)
    return selected
...
def process_prate():
    print("\n== Processing NMME precipitation anomaly (June 2026 init) ==")
...
def process_tmpsfc():
    print("\n== Processing NMME surface-temperature anomaly (June 2026 init) ==")
...
def main():
    print("==================================================")
    print("Compute NMME anomalies -- June 2026 initialization")
    print("==================================================")
    ...
    summary_csv = TABLE_DIR / "nmme_area_mean_anomalies_june_init.csv"
    summary_df.to_csv(summary_csv, index=False)
    print(f"\nSaved area-mean summary: {summary_csv}")

    sst_csv = TABLE_DIR / "nmme_sst_indices_from_tmpsfc_june_init.csv"
    sst_df.to_csv(sst_csv, index=False)
    print(f"Saved SST proxy index table: {sst_csv}")

    print("\nDone. Periods produced: Jul_2026, Aug_2026, Sep_2026, JAS_2026.")
