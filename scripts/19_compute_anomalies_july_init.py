"""
19_compute_anomalies_july_init.py

Purpose
-------
Compute NMME precipitation and surface-temperature anomalies for the
July 2026 initialization, following the same method as
04_compute_anomalies.py / 15_compute_anomalies_june_init.py (mm/day
conversion, monthly totals, seasonal mean/total, cosine-latitude-
weighted area means).

Target months
-------------
The July 2026 init's first available forecast target is AUGUST 2026
(verified against the target-month coordinate: index 0 == 798, the
same absolute value that was empirically August in the May- and
June-init files). Within this project's Kiremt/JJAS scope that leaves
only August and September 2026, plus the two-month AS (Aug-Sep) season.
There is no July, JJA, JAS, or JJAS in this branch.

Outputs
-------
    outputs/netcdf/nmme_anomalies_july_init/
    outputs/tables/nmme_area_mean_anomalies_july_init.csv

Run from project root:
    python scripts\\19_compute_anomalies_july_init.py
"""
...
NMME_PRATE_FILE = NMME_ORG_DIR / "nmme_NMME_prate_202607_ENSMEAN_anom.nc"
NMME_TMPSFC_FILE = NMME_ORG_DIR / "nmme_NMME_tmpsfc_202607_ENSMEAN_anom.nc"

NMME_ANOM_OUT_DIR = NETCDF_OUT_DIR / "nmme_anomalies_july_init"
NMME_ANOM_OUT_DIR.mkdir(parents=True, exist_ok=True)
TABLE_DIR.mkdir(parents=True, exist_ok=True)

# index 0 of the organized July-init file == August 2026 (see module docstring)
TARGET_MONTHS = {
    "Aug_2026": {"index": 0, "month": 8, "days": 31},
    "Sep_2026": {"index": 1, "month": 9, "days": 30},
}

SEASONS = {
    "AS_2026": ["Aug_2026", "Sep_2026"],
}
...
def process_prate():
    print("\n== Processing NMME precipitation anomaly (July 2026 init) ==")
...
def process_tmpsfc():
    print("\n== Processing NMME surface-temperature anomaly (July 2026 init) ==")
...
def main():
    print("==================================================")
    print("Compute NMME anomalies -- July 2026 initialization")
    print("==================================================")
    ...
    summary_csv = TABLE_DIR / "nmme_area_mean_anomalies_july_init.csv"
    summary_df.to_csv(summary_csv, index=False)
    print(f"\nSaved area-mean summary: {summary_csv}")

    sst_csv = TABLE_DIR / "nmme_sst_indices_from_tmpsfc_july_init.csv"
    sst_df.to_csv(sst_csv, index=False)
    print(f"Saved SST proxy index table: {sst_csv}")

    print("\nDone. Periods produced: Aug_2026, Sep_2026, AS_2026.")
